...
def format_tree(
    ...
    node_renderer: Callable[[Tuple[int, Any]], str] = str,
    ...
    def format_nodes() -> Generator[Row, None, None]:
        ...
        def make_row(
            token, node_idx, level, active_levels, fill, replace_token: bool = False
        ):
            pre = "".join(
                style.vertical if l in active_levels else "    "
            ...
            if replace_token:
                ...
                token = node_renderer(node_idx, token)
        ...
        for node_idx, (token, lev) in enumerate(zip(tree.node_data, levels), start=0):
            ...
            if max_nodes is not None and node_idx >= (
                max_nodes - len(active_levels) - 2
            ):
    ...
    return "\n".join(f"{row.pre}{row.fill}{row.node}" for row in format_nodes())
